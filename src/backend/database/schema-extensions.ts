...
export function initializeSchemaExtensions(db: Database.Database): void {
  ...
  const categoryCount = (db.prepare('SELECT COUNT(*) as count FROM expense_categories').get() as any).count;
  if (categoryCount === 0) {
  ...
  const paymentCount = (db.prepare('SELECT COUNT(*) as count FROM payment_methods').get() as any).count;
  if (paymentCount === 0) {
  ...
  const settingsCount = (db.prepare('SELECT COUNT(*) as count FROM invoice_settings').get() as any).count;
  if (settingsCount === 0) {
    db.prepare(`
    ...
    `).run(
      'WALMART',
      'Post Office Road in Stylo Basement',
      '03035577787',
  ...
  const barcodeCount = (db.prepare('SELECT COUNT(*) as count FROM barcode_settings').get() as any).count;
  if (barcodeCount === 0) {
    db.prepare(`
