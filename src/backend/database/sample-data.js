...
exports.deleteSampleData = exports.seedSampleData = void 0;
...
function seedSampleData(db) {
  ...
  `);
    ...
    sampleProducts.forEach((product) => {
        insertProduct.run(product.sku, product.barcode, product.name, product.category, product.purchase_price, product.sale_price, product.quantity, 10);
  ...
  `);
    ...
    sizes.forEach((size) => {
        colors.forEach((color) => {
            ...
            const variantId = db.prepare('SELECT last_insert_rowid() as id').get().id;
  ...
  `);
    ...
    rentalItems.forEach((item) => {
        insertRentalItem.run(productIds[8], item.unit, item.daily, item.weekly, item.monthly, item.deposit, 'excellent');
        const rentalId = db.prepare('SELECT last_insert_rowid() as id').get().id;
  ...
  `);
    ...
    bulkPrices.forEach((bp) => {
        insertBulkPrice.run(productIds[1], bp.minQty, bp.maxQty, bp.price, bp.discount);
        const bulkId = db.prepare('SELECT last_insert_rowid() as id').get().id;
        insertSampleMarker.run('bulk_price', bulkId, `Sample bulk: Min Qty ${bp.minQty}`);
    });
    // ============ CREATE SAMPLE USER FOR TESTING ============
    console.log('\n👤 Creating sample test user...');
    const passwordHash = bcryptjs_1.default.hashSync('test123456', 10);
...
exports.deleteSampleData = deleteSampleData;
