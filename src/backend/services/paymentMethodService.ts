...
export class PaymentMethodService {
  ...
  createPaymentMethod(name: string): PaymentMethod {
    ...
    if (existing) {
      throw new AppError(409, 'Payment method already exists');
    }
    ...
    const methodId = (this.db.prepare('SELECT last_insert_rowid() as id').get() as any).id;
    return this.getPaymentMethod(methodId) as PaymentMethod;
  }
  ...
  getAllPaymentMethods(activeOnly: boolean = true): PaymentMethod[] {
    if (activeOnly) {
      return this.db
  ...
  updatePaymentMethod(id: number, data: { name?: string; active?: number }): PaymentMethod {
    ...
    return this.getPaymentMethod(id) as PaymentMethod;
  }

  // ============ SPLIT PAYMENTS ============

  processSplitPayment(saleId: number, payments: Array<{ methodId: number; amount: number }>): SalePayment[] {
    const sale = this.db
    ...
    if (Math.abs(totalPayment - sale.total_amount) > 0.01) {
      throw new AppError(400, 'Payment amounts do not match sale total');
    }
    ...
    for (const payment of payments) {
      ...
      if (!method) {
        throw new AppError(404, `Payment method ${payment.methodId} not found`);
      }

      stmt.run(saleId, payment.methodId, payment.amount);
      const paymentId = (this.db.prepare('SELECT last_insert_rowid() as id').get() as any).id;
      salePayments.push(this.getSalePayment(paymentId) as SalePayment);
    }
  ...
  getPaymentMethodTrends(monthsBack: number = 12): any[] {
