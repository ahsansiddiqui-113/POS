...
export class StockAlertService {
  ...
  getAlertSettings(productId: number): StockAlertSetting | null {
    return this.db
      ...
      .get(productId) as StockAlertSetting | null;
  }
  ...
  createAlertSetting(data: {
    ...
    if (existing) {
      throw new AppError(409, 'Alert settings already exist for this product');
    }
    ...
    stmt.run(
      ...
      data.reorderSupplierId || null
    );

    return this.getAlertSettings(data.productId) as StockAlertSetting;
  }
  ...
  updateAlertSetting(productId: number, data: {
    ...
    if (!setting) {
      throw new AppError(404, 'Alert settings not found');
    }
    ...
    return this.getAlertSettings(productId) as StockAlertSetting;
  }
  ...
  createStockAlert(productId: number, alertType: 'low_stock' | 'expiry_warning', notes?: string): StockAlert {
    ...
    const alertId = (this.db.prepare('SELECT last_insert_rowid() as id').get() as any).id;
    return this.getAlert(alertId) as StockAlert;
  }

  getAlert(id: number): StockAlert | null {
    return this.db
      .prepare('SELECT * FROM stock_alerts WHERE id = ?')
      .get(id) as StockAlert | null;
  }
  ...
  getExpiringProducts(daysUntilExpiry: number = 30): any[] {
    ...
    return this.db
      ...
      `)
      .all(today, today, expiryDateStr) as any[];
  }
  ...
  generateReorderList(categoryFilter?: string): any[] {
    ...
    if (categoryFilter) {
      ...
      return this.db.prepare(query).all(categoryFilter) as any[];
    }
  ...
  checkAndCreateAlerts(): { created: number; skipped: number } {
    let createdCount = 0;
    let skippedCount = 0;
    ...
    for (const product of lowStockProducts) {
      const existingAlert = (this.db
        ...
        .get(product.id, 'low_stock') as any);
    ...
    for (const product of expiringProducts) {
      const existingAlert = (this.db
        .prepare('SELECT id FROM stock_alerts WHERE product_id = ? AND alert_type = ? AND resolved = 0')
        .get(product.id, 'expiry_warning') as any);
