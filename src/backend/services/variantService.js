"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.variantService = exports.VariantService = void 0;
const db_1 = require("../database/db");
const errorHandler_1 = require("../middleware/errorHandler");
class VariantService {
    constructor() {
        this.db = (0, db_1.getDatabase)();
    }
    createVariant(data) {
        // Check for duplicate SKU
        const existing = this.db
            .prepare('SELECT id FROM product_variants WHERE variant_sku = ?')
            .get(data.variant_sku);
        if (existing) {
            throw new errorHandler_1.AppError(409, 'Variant SKU already exists');
        }
        const stmt = this.db.prepare(`
      INSERT INTO product_variants (
        product_id, size, color, variant_sku, quantity_available, low_stock_threshold
      )
      VALUES (?, ?, ?, ?, ?, ?)
    `);
        stmt.run(data.product_id, data.size || null, data.color || null, data.variant_sku, data.quantity_available, data.low_stock_threshold || 5);
        return this.getVariant(this.db.prepare('SELECT last_insert_rowid() as id').get());
    }
    getVariant(id) {
        return this.db
            .prepare('SELECT * FROM product_variants WHERE id = ?')
            .get(id);
    }
    getVariantBySku(sku) {
        return this.db
            .prepare('SELECT * FROM product_variants WHERE variant_sku = ?')
            .get(sku);
    }
    getProductVariants(productId) {
        return this.db
            .prepare('SELECT * FROM product_variants WHERE product_id = ? ORDER BY size, color')
            .all(productId);
    }
    updateVariantStock(id, quantityChange) {
        const variant = this.getVariant(id);
        if (!variant) {
            throw new errorHandler_1.AppError(404, 'Variant not found');
        }
        const newQuantity = variant.quantity_available + quantityChange;
        if (newQuantity < 0) {
            throw new errorHandler_1.AppError(400, 'Insufficient stock for this variant');
        }
        this.db
            .prepare('UPDATE product_variants SET quantity_available = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?')
            .run(newQuantity, id);
    }
    updateVariant(id, data) {
        const updates = [];
        const values = [];
        if (data.quantity_available !== undefined) {
            updates.push('quantity_available = ?');
            values.push(data.quantity_available);
        }
        if (data.low_stock_threshold !== undefined) {
            updates.push('low_stock_threshold = ?');
            values.push(data.low_stock_threshold);
        }
        if (data.size !== undefined) {
            updates.push('size = ?');
            values.push(data.size);
        }
        if (data.color !== undefined) {
            updates.push('color = ?');
            values.push(data.color);
        }
        if (updates.length === 0)
            return;
        updates.push('updated_at = CURRENT_TIMESTAMP');
        values.push(id);
        const query = `UPDATE product_variants SET ${updates.join(', ')} WHERE id = ?`;
        this.db.prepare(query).run(...values);
    }
    deleteVariant(id) {
        this.db.prepare('DELETE FROM product_variants WHERE id = ?').run(id);
    }
    getLowStockVariants(threshold) {
        const query = threshold
            ? 'SELECT * FROM product_variants WHERE quantity_available <= ? ORDER BY quantity_available ASC'
            : 'SELECT * FROM product_variants WHERE quantity_available <= low_stock_threshold ORDER BY quantity_available ASC';
        return this.db
            .prepare(query)
            .all(threshold);
    }
    getTotalVariantStock(productId) {
        const result = this.db
            .prepare('SELECT COALESCE(SUM(quantity_available), 0) as total FROM product_variants WHERE product_id = ?')
            .get(productId);
        return result.total;
    }
    // Get all sizes available for a product
    getAvailableSizes(productId) {
        const results = this.db
            .prepare('SELECT DISTINCT size FROM product_variants WHERE product_id = ? AND size IS NOT NULL ORDER BY size')
            .all(productId);
        return results.map((r) => r.size);
    }
    // Get all colors available for a product
    getAvailableColors(productId) {
        const results = this.db
            .prepare('SELECT DISTINCT color FROM product_variants WHERE product_id = ? AND color IS NOT NULL ORDER BY color')
            .all(productId);
        return results.map((r) => r.color);
    }
    // Get variant by size and color
    getVariantBySizeColor(productId, size, color) {
        return this.db
            .prepare('SELECT * FROM product_variants WHERE product_id = ? AND size = ? AND color = ?')
            .get(productId, size || null, color || null);
    }
}
exports.VariantService = VariantService;
exports.variantService = new VariantService();
//# sourceMappingURL=variantService.js.map